...
int main(int argc, const char *argv[])
{
    ...
    //use absolute path for local debugging
    //dataPath = "/home/level-3/SFND/SFND_3D_Object_Tracking/";
    ...
    if (print_to_file)
    {
        string filename = dataPath + "src/results/data";
        ...
        for (auto it_descriptor = std::begin(descriptor); it_descriptor != std::end(descriptor); ++it_descriptor)
        {
            ...
            try
            {
                ...
                for (size_t imgIndex = 0; imgIndex <= imgEndIndex - imgStartIndex; imgIndex += imgStepWidth)
                {
                    ...
                    if (verbose)
                        std::cout << "#2 : DETECT & CLASSIFY OBJECTS done" << endl;
                    ...
                    string detectorType = *it_detector; //"SHITOMASI";
                    bool visualize_result = false;
                    ...
                    if (verbose)
                        std::cout << detectorType << " detection with n=" << keypoints.size() << " keypoints in " << 1000 * t_detect / 1.0 << " ms" << endl;
                    ...
                    if (verbose)
                        std::cout << "#5 : DETECT KEYPOINTS done" << endl;

                    /* EXTRACT KEYPOINT DESCRIPTORS */

                    cv::Mat descriptors;
                    string descriptorType = *it_descriptor; // "BRISK"; // BRISK, BRIEF, ORB, FREAK, AKAZE, SIFT
                    ...
                    if (verbose)
                        std::cout << "#6 : EXTRACT DESCRIPTORS done" << endl;
                    ...
                    if (dataBuffer.size() > 1) // wait until at least two images have been processed
                    {
                        ...
                        if (verbose)
                            std::cout << "#7 : MATCH KEYPOINT DESCRIPTORS done" << endl;
                        ...
                        if (verbose)
                            std::cout << "#8 : TRACK 3D OBJECT BOUNDING BOXES done" << endl;
                        ...
                        for (auto it1 = (dataBuffer.end() - 1)->bbMatches.begin(); it1 != (dataBuffer.end() - 1)->bbMatches.end(); ++it1)
                        {
                            ...
                            if (currBB->lidarPoints.size() > 0 && prevBB->lidarPoints.size() > 0) // only compute TTC if we have Lidar points
                            {
                                ...
                                if (bVis)
                                {
                                    ...
                                    //cv::drawKeypoints(visImage, currBB->keypoints, visImage, cv::Scalar::all(-1), cv::DrawMatchesFlags::DRAW_RICH_KEYPOINTS);
                                    cv::drawKeypoints(visImage, currBB->keypoints, visImage, cv::Scalar::all(-1), cv::DrawMatchesFlags::DRAW_RICH_KEYPOINTS);
                    ...
                    // exclude writing details of first image as there will be no matches
                    if (imgNumber.str() == "0000")
                    {
                        first_img = true;
                    }
                    else
                    ...
                    if (print_to_file && first_img != true)
                    {
                        myfile << detectorType << ","
                               << 1000 * t_detect / 1.0 << ","
                               << detected_keypoints << ","
    ...
    std::cout << "Errors : " << num_errors << endl;
